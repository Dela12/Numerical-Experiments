...
# ╔═╡ b3919293-c41b-4642-8e88-cbfe010dd5d9
begin 
	using PlutoUI,ExtendableGrids,ExtendableSparse,ForwardDiff,LinearAlgebra, SparseArrays,VoronoiFVM,GridVisualize, PlutoVista
	using HypertextLiteral
	GridVisualize.default_plotter!(PlutoVista)
end;

# ╔═╡ 10f5aeaf-a23a-45ec-8226-5b1a52ac5a88
begin
	using  ImageShow, FileIO, ImageIO
end

# ╔═╡ 9190dec4-c592-4a5d-85de-099702644c8a
md"""
### Finite Difference ::  Motivation

The basic idea of the method will be to replace differential operators by a finite
difference approximation, which will lead to a large sparse system of linear or nonlinear equations.

With the finite difference method, we descretize the partial differential equation by replacing partial derivatives with their approximation that is finite differences.

We have the following approximations :: 

forward difference quotient :
$\begin{align}
   D^{+}\phi(x) :=& \frac{\phi(x+h)-\phi(x)}{h} \end{align}$

backward difference quotient:
$\begin{align}
   D^{-}\phi(x) :=& \frac{\phi(x)-\phi(x-h)}{h}\end{align}$

central difference quotient:
$\begin{align}
   D^{0}\phi(x) :=& \frac{\phi(x+h)-\phi(x-h)}{2h}\end{align}$

for $$x \in \bar{\Omega}$$ and a sufficiently small mesh size $$h \ge 0$$ so that $$x+h \in \bar{\Omega}$$. The difference operators are linear and fufill,

$\begin{align}
   D^{0}(ku + v)(x) = kD^{0}u(x) + D^{0}v(x)\end{align}$  (NB: Same applies to $$G^{+}$$ and $$G^{-}$$).

For the approximation of the second derivative
we apply two of the operators. The most classical one reads,

$\begin{align}
   D^{+}D^{-}\phi(x) = \frac{\phi(x+h)-2\phi(x)+ \phi(x-h)}{h^2} \end{align}$
"""

# ╔═╡ 068a29a6-2596-43b7-93b8-87a4ec04d707
md"""
...
Now, let $\Omega = (v_1,v_2)$ such that $v_1 = 0$ and $v_2 = 1$. Consider the following problem depicting the Poisson-Boltzmann and Poisson-Bikerman models,

$\begin{align}
    -\varepsilon \Delta \phi +(e^{2ϕ}-e^{-2ϕ}) &= 0 \hspace{0.5cm} \text{in } \Omega \in \mathbb{R}^1 \notag\\ 
    \phi_1(v_1) &= \beta_1 \hspace{0.5cm} \text{on } \partial \Omega \\ \phi_2(v_2) &= \beta_2 \hspace{0.5cm} \text{on } \partial \Omega \notag
\end{align}$

which describes the electrical potential $\phi \in \Omega$. The term $q(\phi) = (e^{2ϕ}-e^{-2ϕ})$ is the charge density which generates the electrical potential in a region with a position-dependent as well as a piecewise constant dielectric function $\varepsilon$. We shall solve the problem in the finite domain $\Omega = (0,1)$ with Dirichlet boundary condition specified on $\partial\Omega$ so that $$\bar{\Omega} = \Omega \cup \partial \Omega = [0,1]$$.
"""

# ╔═╡ 459824e7-eeb2-4cd1-99b9-eb15b810b03f
md"""
#### Diagram

one-D domain discretization
"""

# ╔═╡ 6b57319e-3325-4493-afb1-9c687bff70d2
begin
    url = "https://github.com/Dela12/Diagram/blob/main/findiff.png"
	findiff_filename = download(url)
	# findiff = load(findiff_filename)	
end

# ╔═╡ b728b054-df0e-4cf5-915d-d98f20c0185d
Resource("https://github.com/Dela12/Diagram/blob/main/findiff.png")

# ╔═╡ 161ba816-eea1-40bd-a52f-abdb69c07ad4
md"""
#### Discretizing

We dicuss now the discretization procedure for our problem.

###### Notations
- Let $i = 0,1,2...,n+1$
- discrete set of points/nodes $x_i$
- Let $\phi_i$ denote the discrete solution at $x_i$
- Exact solution $\phi$

We shall seek an approximate solution only at discrete set of points $$x_i$$. Our goal is to obtain $\phi_i \approx \phi(x_i)$.

###### step 1
We start by discretizing the domain. Let $\Omega = (v_1,v_2)$ such that $v_1 = 0$ and $v_2 = 1$. Then we create a mesh (distance between nodes) $$h$$ of $$n$$ nodes

$\begin{align} 
   h = \frac{v_2-v_1}{n-1} 
\end{align}$

and we can compute the nodes as $x_i = i \times h$

###### step 2
The nodal points corresponding to $i = 0$ and $i = n$ are boundary points where we specified our boundary conditions. That's for our Dirichlet boundary conditions, we have;

$\begin{align}
   \phi_1(v_1) = \beta_1(x_0)  \\
    \phi_2(v_2) = \beta_1(x_n)
\end{align}$

which represnts our first and last equations repectively.
We will replace the operator $-\varepsilon \Delta \phi = -\varepsilon \phi''$ by the second order difference approximation:

$\begin{align}
   -\varepsilon \phi^{''}(x_i) = \frac{\varepsilon}{{h^2}} [-\phi(x_{i-1}) + 2\phi(x_i) - \phi(x_{i+1})]
\end{align}$

###### step 3
Our problem holds in the interior with the reaction function. Consequently, the approximated discrete Pde problem reads:


$\begin{align}
  \frac{\varepsilon}{h^2} [-\phi_{i-1}+ 2\phi_i - \phi_{i+1}] - q(\phi(x_i)) = 0
\end{align}$

Therefore, at each node $x_i$ we have an explicit formula situated at the boundary points or an implicit nonlinear equation for the interior points made up of the unknown approximate solution $\phi_i$.
In summary, the conditions on the boundary and the interior points gives a nonlinear equation of the form;
...
# ╔═╡ a4a5fedf-948b-43d5-adee-f43714a417ff
md"""
#### Implementation

We employ the Julia package [VoronoiFVM.jl](https://j-fu.github.io/VoronoiFVM.jl) which provides a solver for nonlinear partial differential equations. To solve the above problem we implement the following.

"""

# ╔═╡ aaa6b9a8-5060-4173-8eb3-2f839157043a
md"""
Grid in domain $\Omega=(0,1)$ consisting of n=$(@bind n Scrubbable(10:10:101,default=10)) points.
"""

# ╔═╡ b31fbdb3-6b9f-4397-90e0-4341c7fbb2de
begin
	#v₁ = 0
	#v₂ = 1
	# v₂-v₁=1-0=1 ; h=v₂-v₁/(n-1) 
...
# ╔═╡ 3c525939-5f47-48fc-9161-0ba822ea61e9
ε= 1.0e-3

# ╔═╡ fc836eac-02ce-49a8-891a-2e7194f960cf
spec_idx = 1

# ╔═╡ d286014d-0af6-4022-8790-88bd50443c81
function flux!(q,ϕ, edge)
	q[spec_idx]= ε*(ϕ[spec_idx,1]-ϕ[spec_idx,2])
end

# ╔═╡ a9126def-d8e3-4f31-8fd3-342990784204
function reaction!(q,ϕ,node)
        q[spec_idx]= (exp(2ϕ[spec_idx]) - exp(-2ϕ[spec_idx]))
    end

# ╔═╡ bd3d1ae7-0854-46ea-bed4-93daf05880e7
function storage!(q,ϕ,node)
    q[1]=ϕ[1]
end

# ╔═╡ 423ea164-759c-489b-a922-734161c753f8
...
# ╔═╡ 46596464-0e41-4ca0-984d-09e0dc820330
begin
...
# ╔═╡ f5f0b954-4d92-47e7-8ba7-180fb9e10cd5
scalarplot(grid,solution[1,:])
