...
Now, let $\Omega = (v_1,v_2)$ such that $v_1 = 0$ and $v_2 = 1$. From equation above, consider the following problem depicting the Poisson-Boltzmann and Poisson-Bikerman models,

$\begin{align}
    -\varepsilon \Delta \phi +(e^{2ϕ}-e^{-2ϕ}) &= 0 \hspace{0.8cm} \text{in } \Omega : [0,1] \notag\\ 
...
Here, we implement the differnece operator $$-\varepsilon \Delta \phi +(e^{2ϕ}-e^{-2ϕ}) = 0$$. We begin by first generating our grid using 96 discretizations points. 
"""

# ╔═╡ ee9a18e6-1aa2-4635-b86b-5240ac74e907
begin
	const spec_idx=1
	ε= 1.0e-3
	β₁=0
...
md"""
 ...
 Scrubbable(96:10:101,default=96)) points.
"""
...
# ╔═╡ 61539076-5b60-4fb5-83d1-ab3c14e78707
md"""
###### Next, we set up a finite difference operator for $$-\varepsilon \Delta \phi$$
"""

# ╔═╡ de4971fa-8f4c-4858-bbbd-863553c40957
function Foperator(g,ϕ)      #g = output, ϕ = input
    h=1/(n-1)
...
# ╔═╡ c3cda1a7-0248-4290-bba8-2bcc7133160b
...
# ╔═╡ 7d64730e-0cea-4154-bad8-f91315e6931d
md"""
###### We define the flux, storage and rection terms respectively.
"""

# ╔═╡ a4677b53-6794-48c3-96d4-a2aff3b701f1
function diffusion_source!(q,node)
	q[spec_idx]=0
end
...
# ╔═╡ 0eb4fd27-6b88-4173-9e73-a98579be18b2
md"""
##### Jacobian
"""

# ╔═╡ 7323b8f9-86e1-4c0f-bde1-ece0f092912c
begin
	 jac_ext = ExtendableSparseMatrix(n, n);
	 #@time begin
	        ForwardDiff.jacobian!(jac_ext, Foperator, ones(n), ones(n));
	        flush!(jac_ext)
	#end

	jac = spzeros(n, n);
	#@time begin
	      ForwardDiff.jacobian!(jac, Foperator, ones(n), ones(n));
	      all(jac .== jac_ext)
	#end
end

# ╔═╡ f35e118a-dca7-4f64-a174-a0a6f0cee997
#DataFrame(Matrix(jac_ext),:auto)

#DataFrame(Matrix(jac),:auto)

# ╔═╡ 2e67c011-e660-40d3-8a03-4f4abc32af1a
md"""
##### Set up for VoronoiFVM which uses atomic assembly
"""

# ╔═╡ 8a597618-9298-4f6a-8446-3af517811663
scalarplot(grid,solution[1,:],size=(500,200),title="solution ϕ", ylabel="ϕ")

# ╔═╡ 69964f30-af6f-40bd-9652-fbfe23b912f2
begin
...
md"""
###### Random function to collect the time values.
"""
...
begin
	function elapsed()
		...
		f(2)
	  for n=10:10:200
		t=minimum([@elapsed atomic(n) for i=1:500])
		  push!(times,t)
	  end
		times
	end
...
begin
	function belapsed()
		times=[]
	  for n=10:10:200
		t=@belapsed atomic($n) seconds=0.1
		  push!(times,t)
	  end
		times
	end
...
let
   clf()
   PyPlot.plot(A_elapsed,label="@elapsed")
  PyPlot.plot(A_belapsed,label="@belapsed")
	PyPlot.legend()
	
	gcf()
end

# ╔═╡ d66f6dd9-5cc5-4b92-a35c-66f92befca5d
begin
	physics=VoronoiFVM.Physics(flux=flux!, storage=storage!,reaction=reaction!)
	systemA=VoronoiFVM.System(grid; 
		flux=flux!, 
		storage=storage!, 
	...
	system=VoronoiFVM.System(grid,physics,species=[spec_idx],bcondition=dirichlet_bc!,unknown_storage=:sparse)
	    enable_species!(system,1,[1])
	    solutionA=unknowns(system,inival=1.0)
	    oldsol=unknowns(system,inival=1.0)
	    residual=unknowns(system,inival=1.0)
	    tstep=1.0e100 # large timestep aproximates stationary problem
	    
	   # @time begin
	        VoronoiFVM.eval_and_assemble(system,solution,oldsol,residual,0.0,Inf,0.0,zeros(0))
	flush!(system.matrix)
	    #end
	    
	    all(system.matrix.≈jac_ext)
end

# ╔═╡ 7312e601-c9b7-4b50-b1e8-74aefd36ec66
begin
